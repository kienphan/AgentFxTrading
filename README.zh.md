...
- [架构](#-架构)
- [快速开始](#-快速开始)
- [Docker 实例（全部 45 条）](docs/docker-instances.md)
- [交易策略](#-交易策略)
- [配置](#-配置)
- [API文档](#-api文档)
- [开发](#-开发)
...
4. **运行多品种 Docker 容器**:
   ...
   全部 **45** 条可直接运行的命令（15 个品种 × 3 个策略）已移至独立文件：

   **📖 [docs/docker-instances.md](docs/docker-instances.md)**

   该文件完整列出 Setup Instances 网格中的每一个单元格 —— TMS+ORB、Asian Range Judas Sweep 和 FlowRSI，覆盖
   XAUUSD、EURUSD、GBPUSD、USDJPY、GBPJPY、EURJPY、USDCAD、AUDUSD、AUDJPY、US30、USTEC、DE40、UK100、BTCUSD 和
   ETHUSD —— 并说明需要替换哪些占位符、容器命名规则，以及如何让模拟盘与实盘并行运行。

   示例：纽约时段的 XAUUSD TMS+ORB：
