...
- **Модель 1: Прямой Импульсный Пробой (Direct Breakout)**: Цена уверенно закрывается за пределами Opening Range в пределах окна входа ($\le 5$ свечей).
- **Модель 2: Ретест Пробоя + TDI Bounce (Pullback Continuation)**: При устаревшем пробое ($> 5$ свечей) вход разрешен только при наличии подтвержденного сигнала **TDI Bounce** (`tdi_bounce_bull` / `tdi_bounce_bear`) И структурного подтверждения цены около 5 EMA (`price_above_ema` для BUY / `price_below_ema` для SELL), что исключает входы на истощении движения.
...
- **Пространство для Дыхания Позиции и Giveback Floor**: Предоставляет позиции запас хода при нормальных внутридневных колебаниях (особенно на Криптовалютах и Индексах). Отслеживает пиковую прибыль ($MFE$) на каждом тике, активируя защиту от отката только при значительной прибыли ($\ge 1.5\times$ ATR или после достижения BE) при подтвержденном развороте.
### Правила Входа
...
| Прибыль ≥ 1.2x ATR | Переместить SL на breakeven (+0.1x ATR фиксации) |
| Прибыль ≥ 2.0x ATR | Trail SL 1.0x ATR |
| Откат ≥ 1.0x ATR (после достижения BE) | Автозакрытие (Защита от максимального отката) |
...
| Breakeven Trigger | 1.2x ATR | Прибыль для перевода в безубыток (ATR) |
| Breakeven Offset | 0.1x ATR | Зафиксированная прибыль при безубытке (ATR) |
| Trail Trigger | 2.0x ATR | Прибыль для запуска трейлинга (ATR) |
| Trail Distance | 1.0x ATR | Дистанция трейлинг-стопа от цены (ATR) |
